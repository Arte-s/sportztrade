...
import {TF, CDelayer} from "./Time";
import {
	CreateMyChartArea, DefaultChartColor,
	DrawCandleChart,
	IChartApi,
	IChartMarker,
	ISeriesApi, LineData,
	scrollChartToTime, SeriesDataItemTypeMap, SeriesType
} from "./Chart/Chart";
import {ISymbolQuotes} from "./Symbol";

import $ from "./jquery";

import {sleepAsync, ICancelToken} from "./Common";

import * as lib from "./Common";
import {MyGeneticParams} from "./Optimizer";

//import * as MsTrade from "./Data_MSTrade"
import {IHistorySource} from "./HistoryLoader";
import {TradeStatistics} from "./TradeStatistics";

export {CParamStepper} from "./Strategy";
...
export async function Test2 (
	testerInfo :ITesterInfo,   // Инфа о тестировании
	priceChart :IChartApi,  // элемент графика котировок
	equityChart :IChartApi,  // элемент графика эквити
	onTrades? :(trades :readonly TradeData[])=>void,  // обработчик сделок
	getSpeed? : ()=>number,  // получение значения скорости тестирования
	onProgress? : (percent :number)=>void  // обработчик прогресса выполнения
) : Promise<[TradeStatistics, CTradeBar[]]>
{
...
//let result= await RunTest (testerInfo,  undefined /*(tick, indicators)=>console.log("tick: ",tick)*/);//,  "chartDiv");
	...
	function MyEquityChart(api :IChartApi) {
	...
	function createNewSeriesWrapper<TSeriesType extends SeriesType>(newSeries :ISeriesApi<TSeriesType>) {
		let _empty= true;
		return {
			addData(data : readonly SeriesDataItemTypeMap[TSeriesType][] | SeriesDataItemTypeMap[TSeriesType]) {
...
//	const createNewLineSeriesWrapper = createNewSeriesWrapper<"Line">();
	...
	*/
	...
	// Добавить сделку
	function AddTradeToArrays(time :const_Date, volume :number, tradePrice :number, volumeTotal :number, markers :IChartMarker[], trades :TradeData[]) {
