...
class CStrategySignaller_LanaV implements IStrategySignaller {
    input:tInputLanaV=
        {
            autoLot: 0,
            start: 0,
    ...
    onNewBars (bars: readonly CBar[]): void {
        ...
        const {cTime,coefficient,minStep_percent,onlyUp,
            start,autoLot,minTime,minOrder,prevPercent}:tInputLanaV			= input
...
class CStrategy_LanaV implements IStrategy {
    readonly paramInfo = [
    ...
    getSignaller(param: readonly number[], tf :TF) {
        ...
        return new CStrategySignaller_LanaV(
            {
    ...
    static readonly instance = new CStrategy_LanaV();
}

export const Strategy_Lana3 = CStrategy_LanaV.instance;
...
class CStrategy_LanaVol implements IStrategy {
    readonly paramInfo = [
        Param("start balance", {min: 1, step: 1, progressive: true }, 100, {start: 100, end: 1000000, static: true}),
        //Param("no work", paramType_Period, 100, {start: 1, end: 100, step: 1}),
        Param("Превышение обьема", {min:0, max:50, step: 0.1, progressive: true }, 10),
        Param("Средняя предыдущего обьема",{min:0, max:10, step: 1}, 1),
        Param("Среднее текущего обьема", {min:0, max:10, step: 1}, 1),
    ...
    static readonly instance = new CStrategy_LanaVol();
}

export const Strategy_LanaV = CStrategy_LanaVol.instance;
