import {deepCloneMutable, isDate} from "../Nav/Common";
...
type ParamType= "number"|"string"|"boolean"|"time";
...
interface IParamBase0 { //extends Readonly<{
    ...
    enabled? : boolean|undefined;  // включён ли параметр
...
export interface IParamGroup extends IParamBaseDefault {
    value: IParams;
    range? : undefined;
    enabled? : boolean;
    type? :undefined;
...
export type IParam = ParamTime | ParamNum | ParamEnum<number> | ParamEnum<string> | ParamBoolean | boolean | number | IParamGroup | (()=>any); // & {[key in "name"|"value"|"range"|"enabled"] : any};

type IParamExt<T> = T extends IParamNum|IParamString|IParamBoolean|boolean|IParamGroup|(()=>any) ? T : T extends IParamEnum2<infer TRange> ? T : never;

export type IParamReadonly = ReadonlyFull<IParam>

// @ts-ignore
export function isParamBase(param :IParam) : param is IParamBase;
// // @ts-ignore
...
export function enableAllParams<T extends IParamsReadonly> (params :T, enabled=true) {
    ...
    for(let [key,param] of iterateParams(paramsInfoClone)) {
        if (isParamBase(param) && param.enabled!=undefined) param.enabled= enabled;
    }
...
export type SimpleParamsMutable<T> =
    T extends const_Date ? const_Date
    : Mutable<{
        [key in keyof T] :
...
export function GetSimpleParams<T extends ReadonlyFull<IParams>>(params : T) //: SimpleT<T>
    ...
    for(let key in params) {
        let param : IParamReadonly = params[key];// as IParamReadonly;
        simpleParams[key]=
...
function convert_(valuesObj :{[key :string] :any}, srcObj : IParamsReadonly | readonly any[]) {
    ...
    for(let key in srcObj as IParamsReadonly) {
        let srcval : IParamReadonly = srcObj[key]; //let aaa= resObj[key];
        resObj[key]= srcval;
        let val= valuesObj[key];
        if (val==undefined) {
            if (typeof srcval=="object" && (srcval as IParamBase).enabled!=undefined)
                resObj[key]= {...srcval, enabled: false } as IParamReadonly;
            continue;
        }
